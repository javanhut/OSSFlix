    ...
    return (
      <div style={{
        ...
        <div style={{ marginBottom: "12px", textAlign: "center" }}>
          <h1 style={{
            fontSize: "2.5rem", fontWeight: 800, color: "#fff", margin: 0,
            letterSpacing: "-1px",
        ...
        <div style={{
          ...
          <div style={{ marginBottom: "12px" }}>
            ...
            <input
              type="password" placeholder={setup ? "Admin password" : "Create admin password"}
              value={password} onChange={(e) => setPassword(e.target.value)}
              onKeyDown={(e) => { if (e.key === "Enter") { setup ? handleLogin() : handleSetup(); } }}
          ...
          <button
            ...
            style={{
              ...
              opacity: submitting ? 0.7 : 1, transition: "opacity 0.2s ease",
            }}
          >
          ...
          <button
            ...
            style={{
              ...
              fontSize: "0.8rem", cursor: "pointer", padding: "14px 0 0",
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          ...
          <button
            ...
            style={{ ...css.btn, fontSize: "0.8rem", padding: "8px 14px", background: "rgba(239,68,68,0.15)", color: "#ef4444" }}
          >
            Logout
          </button>
        </div>
      ...
      <div style={{
        ...
        <button style={tabStyle(activeTab === "media")} onClick={() => setActiveTab("media")}>Media & Services</button>
        <button style={tabStyle(activeTab === "addmedia")} onClick={() => setActiveTab("addmedia")}>Add Media</button>
        <button style={tabStyle(activeTab === "migrator")} onClick={() => setActiveTab("migrator")}>Migrator</button>
        <button style={tabStyle(activeTab === "accounts")} onClick={() => setActiveTab("accounts")}>Accounts</button>
      </div>
      ...
      <div style={{ maxWidth: "700px", margin: "0 auto", padding: "24px" }}>
        {activeTab === "media" && (
          <>
            {/* Movies Directory */}
            <SectionHeader icon="film" color="#3b82f6" title="Movies Directory" subtitle="Folder containing your movie directories" />
            <input type="text" value={moviesDir} onChange={(e) => setMoviesDir(e.target.value)}
              placeholder="/path/to/movies" style={{ ...css.input, fontFamily: "monospace", fontSize: "0.82rem", marginBottom: "20px" }} />

            {/* TV Shows Directory */}
            <SectionHeader icon="tv" color="#8b5cf6" title="TV Shows Directory" subtitle="Folder containing your TV show directories" />
            <input type="text" value={tvshowsDir} onChange={(e) => setTvshowsDir(e.target.value)}
              placeholder="/path/to/tvshows" style={{ ...css.input, fontFamily: "monospace", fontSize: "0.82rem", marginBottom: "20px" }} />

            <div style={{ borderTop: "1px solid rgba(255,255,255,0.08)", paddingTop: "20px", marginTop: "4px" }}>
              <SectionHeader icon="key" color="#a855f7" title="TMDB API Key" subtitle="Optional. Enables auto-fetching metadata from TMDB." />
              <div style={{ display: "flex", gap: "8px", marginBottom: "20px" }}>
                <input type="password" value={tmdbKey}
                ...
                <button
                  ...
                  onClick={() => {
                    setTmdbTesting(true); setTmdbTestResult(null);
                    fetch(`https://api.themoviedb.org/3/search/movie?api_key=${encodeURIComponent(tmdbKey)}&query=test`)
                      .then(r => r.ok ? setTmdbTestResult({ ok: true, message: "API key is valid!" }) : setTmdbTestResult({ ok: false, message: "Invalid API key" }))
                      .catch(() => setTmdbTestResult({ ok: false, message: "Connection failed" }))
                      .finally(() => setTmdbTesting(false));
                  }}
                >{tmdbTesting ? "Testing..." : "Test"}</button>
              </div>
              {tmdbTestResult && <p style={{ marginTop: "-12px", marginBottom: "16px", fontSize: "0.78rem", fontWeight: 500, color: tmdbTestResult.ok ? "#22c55e" : "#ef4444" }}>{tmdbTestResult.message}</p>}
            </div>

            {/* KaidaDB */}
            <div style={{ borderTop: "1px solid rgba(255,255,255,0.08)", paddingTop: "20px" }}>
              <SectionHeader icon="db" color="#3b82f6" title="KaidaDB Storage" subtitle="Optional. Remote media storage server." />
              <div style={{ display: "flex", gap: "8px", marginBottom: "12px" }}>
                <input type="text" value={kaidadbUrl}
                ...
                >{kaidadbTesting ? "Testing..." : "Test"}</button>
              </div>
              {kaidadbTestResult && <p style={{ marginBottom: "12px", fontSize: "0.78rem", fontWeight: 500, color: kaidadbTestResult.ok ? "#22c55e" : "#ef4444" }}>{kaidadbTestResult.message}</p>}
              {kaidadbUrl.trim() && (
                <div style={{ display: "flex", flexDirection: "column", gap: "8px", marginBottom: "20px" }}>
                  <div>
                    <label style={{ fontSize: "0.75rem", color: "rgba(255,255,255,0.4)", marginBottom: "2px", display: "block" }}>Root Prefix</label>
                    <input type="text" value={kaidadbRootPrefix} onChange={(e) => setKaidadbRootPrefix(e.target.value)}
                      placeholder="leave empty to scan all" style={{ ...css.input, fontFamily: "monospace", fontSize: "0.82rem" }} />
                  </div>
                  <div style={{ display: "flex", gap: "8px" }}>
                    <div style={{ flex: 1 }}>
                      <label style={{ fontSize: "0.75rem", color: "rgba(255,255,255,0.4)", marginBottom: "2px", display: "block" }}>Movies Prefix</label>
                      <input type="text" value={kaidadbMoviesPrefix} onChange={(e) => setKaidadbMoviesPrefix(e.target.value)}
                        placeholder="movies/" style={{ ...css.input, fontFamily: "monospace", fontSize: "0.82rem" }} />
                    </div>
                    <div style={{ flex: 1 }}>
                      <label style={{ fontSize: "0.75rem", color: "rgba(255,255,255,0.4)", marginBottom: "2px", display: "block" }}>TV Shows Prefix</label>
                      <input type="text" value={kaidadbTvshowsPrefix} onChange={(e) => setKaidadbTvshowsPrefix(e.target.value)}
                        placeholder="tv/" style={{ ...css.input, fontFamily: "monospace", fontSize: "0.82rem" }} />
                    </div>
                  </div>
                </div>
              )}
            ...
            {/* SMTP */}
            <div style={{ borderTop: "1px solid rgba(255,255,255,0.08)", paddingTop: "20px" }}>
              <SectionHeader icon="mail" color="#22c55e" title="Email (SMTP)" subtitle="Required for password recovery emails." />
              <div style={{ display: "flex", gap: "8px", marginBottom: "8px" }}>
                <div style={{ flex: 2 }}>
                  <label style={{ fontSize: "0.75rem", color: "rgba(255,255,255,0.4)", marginBottom: "2px", display: "block" }}>SMTP Host</label>
                  <input type="text" value={smtpHost} onChange={(e) => { setSmtpHost(e.target.value); setSmtpTestResult(null); }}
                    placeholder="smtp.gmail.com" style={{ ...css.input, fontFamily: "monospace", fontSize: "0.82rem" }} />
                </div>
                <div style={{ flex: 1 }}>
                  <label style={{ fontSize: "0.75rem", color: "rgba(255,255,255,0.4)", marginBottom: "2px", display: "block" }}>Port</label>
                  <input type="text" value={smtpPort} onChange={(e) => { setSmtpPort(e.target.value.replace(/\D/g, "")); setSmtpTestResult(null); }}
                    placeholder="587" style={{ ...css.input, fontFamily: "monospace", fontSize: "0.82rem" }} />
                </div>
              </div>
              <div style={{ display: "flex", gap: "8px", marginBottom: "8px" }}>
                <div style={{ flex: 1 }}>
                  <label style={{ fontSize: "0.75rem", color: "rgba(255,255,255,0.4)", marginBottom: "2px", display: "block" }}>Username</label>
                  <input type="text" value={smtpUser} onChange={(e) => setSmtpUser(e.target.value)}
                    placeholder="user@gmail.com" style={{ ...css.input, fontFamily: "monospace", fontSize: "0.82rem" }} />
                </div>
                <div style={{ flex: 1 }}>
                  <label style={{ fontSize: "0.75rem", color: "rgba(255,255,255,0.4)", marginBottom: "2px", display: "block" }}>Password</label>
                  <input type="password" value={smtpPass} onChange={(e) => setSmtpPass(e.target.value)}
                    placeholder="app password" style={{ ...css.input, fontFamily: "monospace", fontSize: "0.82rem" }} />
                </div>
              </div>
              <div style={{ marginBottom: "12px" }}>
                <label style={{ fontSize: "0.75rem", color: "rgba(255,255,255,0.4)", marginBottom: "2px", display: "block" }}>From Address</label>
                <div style={{ display: "flex", gap: "8px" }}>
                  <input type="text" value={smtpFrom} onChange={(e) => setSmtpFrom(e.target.value)}
                    placeholder="noreply@yourdomain.com" style={{ ...css.input, flex: 1, fontFamily: "monospace", fontSize: "0.82rem" }} />
                  <button
                    style={{ ...css.btn, ...css.btnSecondary, flexShrink: 0, opacity: (!smtpHost.trim() || !smtpPort.trim()) ? 0.5 : 1 }}
                  ...
                  >{smtpTesting ? "Testing..." : "Test"}</button>
                </div>
              </div>
              {smtpTestResult && <p style={{ marginBottom: "12px", fontSize: "0.78rem", fontWeight: 500, color: smtpTestResult.ok ? "#22c55e" : "#ef4444" }}>{smtpTestResult.message}</p>}
            </div>
                      ...
                      <div key={p.id} style={{
                        ...
                        <button
                          ...
                          style={{
                            ...
                            cursor: "pointer", padding: "4px", fontSize: "0.75rem",
                          }}
...
function SectionHeader({ icon, color, title, subtitle }: { icon: string; color: string; title: string; subtitle: string }) {
  const icons: Record<string, JSX.Element> = {
    film: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><rect x="2" y="2" width="20" height="20" rx="2.18" ry="2.18"/><line x1="7" y1="2" x2="7" y2="22"/><line x1="17" y1="2" x2="17" y2="22"/><line x1="2" y1="12" x2="22" y2="12"/></svg>,
    tv: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><rect x="2" y="7" width="20" height="15" rx="2" ry="2"/><polyline points="17,2 12,7 7,2"/></svg>,
    key: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M21 2l-2 2m-7.61 7.61a5.5 5.5 0 1 1-7.778 7.778 5.5 5.5 0 0 1 7.777-7.777zm0 0L15.5 7.5m0 0l3 3L22 7l-3-3m-3.5 3.5L19 4"/></svg>,
    db: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><ellipse cx="12" cy="5" rx="9" ry="3"/><path d="M21 12c0 1.66-4 3-9 3s-9-1.34-9-3"/><path d="M3 5v14c0 1.66 4 3 9 3s9-1.34 9-3V5"/></svg>,
    mail: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/></svg>,
  };
  return (
